...
/*
 * ogni volta che viene avviata lancia un task asincorno
 * 
 * il task richiede  un WAKE LOCK (tiene sveglio il cell mentre l'op è in corso)
 * 
 * contatta l'endpoint retrieve.jsp del server socialstudy
 * 
 * scarica e salva i msg con/sul il content provider e inserisce una notifica visuale nella notification bar
 * 
 * per il download politica di RETRY ON FAILURE
 */
import java.net.URL;

import socialstudy.android.R;
...
public class RetrieveMessagesService extends Service {
	...
	private void retrieveMessage(int failedAttempts) {
		...
		try {
			...
			Log.e("socialstudy", "errore durante il download dei messaggi", e);
			// Rischedula se ci sono meno di 5 tentativi falliti. RETRY ON FAILURE
			failedAttempts++;
			if (failedAttempts < 5) {
	...
	private class RetrieveMessageTask extends AsyncTask<Integer, Void, Void> {
		...
		protected void onPreExecute() {
			...
			mWakeLock = powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "socialstudy-retrieve-message");
			mWakeLock.acquire();
		}
